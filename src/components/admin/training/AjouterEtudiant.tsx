...
interface AjouterEtudiantProps {
  classe?: Classe;
  onSuccess: () => void;
  onCancel: () => void;
}

export default function AjouterEtudiant({ classe, onSuccess, onCancel }: AjouterEtudiantProps) {
  ...
  const checkCinExists = async (cin: string): Promise<boolean> => {
    try {
      const { data, error } = await supabase
        .from('etudiants')
        .select('id')
        .eq('cin', cin.toUpperCase())
        .single();
  ...
  const checkClassCapacity = async (): Promise<boolean> => {
    try {
      const { data: selectedClass } = await supabase
        .from('classes')
        .select('nombre_places')
      ...
      const { data: inscriptionsData, error } = await supabase
        .from('inscriptions')
        .select('id')
        .eq('classe_id', formData.classe_id);

      if (error) throw error;

      const currentInscriptions = inscriptionsData?.length || 0;
      return currentInscriptions < (selectedClass?.nombre_places || 0);
    } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const cinExists = await checkCinExists(formData.cin);
      if (cinExists) {
        toast({
          title: "Erreur",
          description: "Un étudiant avec ce CIN existe déjà",
          variant: "destructive"
        });
        setLoading(false);
        return;
      ...
      const hasCapacity = await checkClassCapacity();
      if (!hasCapacity) {
        toast({
          title: "Erreur",
          description: "Inscription impossible : cette classe est complète",
          variant: "destructive"
        });
        setLoading(false);
        return;
      ...
      const { data: etudiantResult, error: etudiantError } = await supabase
        .from('etudiants')
        .insert([etudiantData])
        .select()
        .single();
      ...
      const inscriptionData = {
        ...
        statut_inscription: 'en_attente' as const
      };
      ...
      toast({
        title: "Succès",
        description: `L'étudiant ${formData.prenom} ${formData.nom} a été inscrit avec succès !`
      });

      onSuccess();
    } catch (error) {
      console.error('Error creating student and inscription:', error);
      toast({
        title: "Erreur",
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      {/* Section 2: Informations sur l'inscription */}
      <div>
